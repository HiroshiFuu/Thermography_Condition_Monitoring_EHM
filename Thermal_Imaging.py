...
class Thermal_Imaging:
	...
	def update_display(self):		
		...
		frame = pygame.transform.smoothscale(self.get_frame(self.image), (IMAGEDISPLAYWIDTH, IMAGEDISPLAYHEIGHT))
		...
		for i in range(0, 3):
			if selected_images_flag[i]:
				...
				frame = pygame.transform.smoothscale(self.get_cropped_frame(self.image, i), (selected_images_rect[i].w, selected_images_rect[i].h))
		...
		for event in pygame.event.get(): # event handling loop
			...
			if event.type == pygame.QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
				...
				if self.Exit.pressed(pygame.mouse.get_pos()):
					#cv2.imwrite('ref.png', self.image)
					pygame.quit()
					sys.exit()
